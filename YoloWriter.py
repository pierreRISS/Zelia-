...
def write_yolo_annotations(output_dir: str, instance_segmaps: List[np.ndarray],
                           instance_attribute_maps: List[dict],
                           colors: List[np.ndarray], camera_resolution: tuple[int], color_file_format: str = "PNG",
    ...
    image_offset = len(os.listdir(f"{output_dir}/images"))

    # collect all RGB paths
    new_coco_image_paths = []
    new_yolo_label_paths = []
    ...
    for frame in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
        ...
        new_yolo_label_paths.append(f'{output_dir}/{LABEL}/{file_prefix}{frame + image_offset:06d}.txt')

    _CocoWriterUtility.geerate_yolo_anotation(instance_segmaps,
...
class _CocoWriterUtility:
    ...
    @staticmethod
    def geerate_yolo_anotation(inst_segmaps, inst_attribute_maps, image_paths, supercategory,
                                  mask_encoding_format, label_paths, camera_resolution,
        ...
        # i = 0
        for inst_segmap, image_path, instance_2_category_map, label_path in zip(inst_segmaps, image_paths,
                                                                    instance_2_category_maps, label_paths):
            ...
            for inst in instances:
                if inst in instance_2_category_map:
                    ...
                    bbox = {
                        "Class": instance_2_category_map[inst],
                        "Cordonate": _CocoWriterUtility.normalise(_CocoWriterUtility.bbox_from_binary_mask(binary_inst_mask, camera_resolution), camera_resolution),
                    }
                    bboxs.append(bbox)

            with open(label_path, "w") as file:
                for bbox in bboxs:
                    file.write(f"{bbox['Class']} {' '.join(str(item) for item in bbox['Cordonate'])}\n")        
    ...
    @staticmethod
    def normalise(cord: List[float], resolution: tuple[int]):
        return [cord[0] / resolution[0],
                cord[1] / resolution[1],
                cord[2] / resolution[0],
                cord[3] / resolution[1]]
    ...
    @staticmethod
    def bbox_from_binary_mask(binary_mask: np.ndarray, camera_resolution) -> List[int]:
        ...
        """
        0.1# Find all columns and rows that contain 1s
        ...
        h = rmax - rmin + 1
        w = cmax - cmin + 1
        return [float(cmin + (w/2)), float(rmin + (h/2)), float(w), float(h)]
